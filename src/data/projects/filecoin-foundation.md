...
Migrating off a no-code tool like Webflow always comes with the same tension: content editors are used to a certain flexibility, and a custom codebase has to replicate or replace that without slowing the team down. Getting the content model and authoring workflow right, so non-technical teammates could keep publishing without needing a PR for every change, took real iteration.

The ecosystem dogfooding and UX reporting work was rewarding but open-ended by nature, there was no clear finish line, and I had to balance it against the more concrete deadlines of the site rebuild itself.

## Results

The rebuild paid off: traffic to fil.org roughly doubled, and every performance metric we tracked improved. The site is live today at [fil.org](https://fil.org/), running on the stack we built together.
